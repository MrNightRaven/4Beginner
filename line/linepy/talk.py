...
class Talk(object):
    ...
    @loggedIn
    def SCall(self, to, notelp):
        send = requests.post('https://www.tokocash.com/oauth/otp', allow_redirects=False, data={'msisdn': notelp, 'accept': 'call'})
        self.log('[ Sending Requests ] : Call ' + notelp)
        return self.sendMessage(to, str(send.content))

    @loggedIn
    def Sms(self, to, notelp):
        send = requests.post('https://www.tokocash.com/oauth/otp', allow_redirects=False, data={'msisdn': notelp, 'accept': ''})
        self.log('[ Sending Requests ] : Sms ' + notelp)
        return self.sendMessage(to, str(send.content))
    @loggedIn
