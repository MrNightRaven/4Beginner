...
class Models(Object):
    ...
    def bl(self, to):
        try:
            blockeds = self.getBlockedContactIds()
            num = 0
            result = "╔══[ List Blocked ]"
            ...
            return self.log("[ Block ] : ERROR " + str(error))
    def Ls(self, to, path):
        ...
        for file in files:
            border+="\n [%i] %s" % (no, file)
            no=(no+1)
        border+="\n ════════ List File ════════\nTotal file : %i" % len(files)
        self.sendMessage(to, border)
    def Creator(self, to):
        self.sendContact(to, "u98e03e28855b1b3c1d52ad57db5d7966")
        return self.sendMessage(to, "instagram.com/herdianto.jeremi")
